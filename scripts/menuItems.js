export const menuItems = [
  {
    title: '"Crispy Pork" Pad Krapow',
    price: "13.99",
    description: "Fried Pork Belly, Onions, Gai Lan, Thai Sauces",
    image: "./images/CrispyPork.webp",
    alt: "Crispy Pork Pad Krapow",
    category: "main",
  },
  {
    title: "Chicken or beef Pad Krapow",
    price: "11.99 - 13.99",
    description:
      "Minced Pork or chicken, yellow onion, Green Beans, Thai basil, Jalapeno pepper, Thai Seasonings",
    image: "./Images/padKrapow.webp",
    alt: "Pad Krapow image",
    category: "main",
  },
  {
    title: "Thai Curry",
    price: "11.99 - 13.99",
    description: "Chicken or Beef, Green Beans, Red Thai Curry",
    image: "./images/thaiCurry.webp",
    alt: "Thai Curry image",
    category: "main",
  },
  {
    title: "Zucchini Chicken",
    price: "11.99",
    description: "Chicken, Bell Pepper, Zucchini, Peanuts, Sauces",
    image: "./images/zucchiniChicken.webp",
    alt: "Zucchini Chicken image",
    category: "main",
  },
  {
    title: "Thai Cashew Chicken",
    price: "11.99",
    description:
      "Chicken (Breaded), Bell Pepper, Green Onions, Red Onions, Cashews, Thai Sauces",
    image: "./images/thaiCashewChicken.webp",
    alt: "Thai Cashew Chicken image",
    category: "main",
  },
  {
    title: "Pineapple Chicken",
    price: "11.99",
    description:
      "Chicken, Bell Pepper, Onion, Pineapple, Sweet Thai Chili Sauce.",
    image: "./images/pineappleChicken.webp",
    alt: "Pineapple Chicken image",
    category: "main",
  },
  {
    title: "Chicken Pad Thai (Tofu or Vegetarian option)",
    price: "11.99",
    description:
      "Chicken, Pad Thai Noodles, Egg, Green Onion, Tofu, In-house Made Sauce (Peanuts, Lime wedge, and Spicy flakes are separated for your preference)",
    image: "./images/chickenPadThai.webp",
    alt: "Chicken Pad Thai image",
    category: "main",
  },
  {
    title: "Shrimp Pad Thai",
    price: "13.99",
    description:
      "Shrimp, Pad Thai Noodles, Egg, Green Onion, Tofu, In-house Made Sauce (Peanuts, Lime wedge, and Spicy flakes are separated for your preference)",
    image: "./images/shrimpPadThai.webp",
    alt: "Shrimp Pad Thai image",
    category: "main",
  },
  {
    title: "Pad See Eew",
    price: "11.99 - 13.99",
    description:
      "Chicken or Beef, Pad See Eew Noodles, Green Onion, Gai Lan, Thai Sauce, and seasoning",
    image: "./images/padSeeEew.webp",
    alt: "Pad See Eew",
    category: "main",
  },
  {
    title: "Chicken Teriyaki Stir-fry (Vegetarian optional)",
    price: "11.99",
    description:
      "Chicken, Snow Peas, Carrots, Cauliflower, Mushrooms, Cabbage, Soba Noodles, and Teriyaki Sauce.",
    image: "./images/ChickenTeriyaki.webp",
    alt: "Chicken Teriyaki Stir-fry image",
    category: "main",
  },
  {
    title: "Beef Teriyaki Stir-Fry",
    price: "13.99",
    description:
      "Beef, Snow Peas, Carrots, Cauliflower, Mushrooms, Cabbage, Soba Noodles, and Teriyaki Sauce.",
    image: "./images/BeefTeriyaki.webp",
    alt: "Beef Teriyaki Stir-Fry image",
    category: "main",
  },
  {
    title: "Shrimp & Broccoli",
    price: "13.99",
    description: "Shrimp, Broccoli, Carrots, Seasoning, and Sauce.",
    image: "./images/shrimpandbroccoli.webp",
    alt: "Shrimp & Broccoli image",
    category: "main",
  },
  {
    title: "Black Pepper Stir-fry",
    price: "11.99 - 13.99",
    description:
      "Chicken or Beef, Red Bell Pepper, Yellow Onions, Green Onions, Spiced with Pepper Corn (crushed), Seasoning, and Sauces.",
    image: "./images/blackPepper.webp",
    alt: "Black Pepper Stir-fry image",
    category: "main",
  },
  {
    title: "Vegetarian Stir-Fry (Tofu Optional)",
    price: "11.99 - 13.99",
    description:
      "Snow Peas, Carrots, Cauliflower, Mushrooms, Seasoning, and Sauces.",
    image: "./images/vegitarian.webp",
    alt: "Vegetarian Stir-Fry image",
    category: "main",
  },
  {
    title: "Drunken Noodle (Pad Kee Mao) (Tofu Optional)",
    price: "11.99 - 13.99",
    description:
      "Chicken, Shrimp, or Beef, Wide Noodles, Gai Lan, Carrots, Pickled Kachai, Yellow Onion, Jalapeno, Thai Seasoning, and Sauces.",
    image: "./images/Drunken Noodle.jpg",
    alt: "Drunken Noodle image",
    category: "main",
  },
  {
    title: "Tom Yum",
    price: "12.99 - 14.99",
    description: "Shrimp, Coconut Milk, Tomato, Lemon leaf, Thai Pepper",
    image: "./images/tomYum.webp",
    alt: "Tom Yum Soup",
    category: "soups",
  },
  {
    title: "Shrimp Fried Rice",
    price: "13.99",
    description: "Shrimp, Rice, Egg, Carrots, Thai Seasoning",
    image: "./images/shrimpFriedRice.webp",
    alt: "Shrimp Fried Rice",
    category: "sides",
  },
  {
    title: "Green Thai Curry (Tofu Optional)",
    price: "12.99",
    description: "Chicken, Green Curry, Coconut Milk, Thai Basil, Bell Peppers",
    image: "./images/greenCurry.webp",
    alt: "Green Thai Curry image",
    category: "soups",
  },
  {
    title: "Tom Kha",
    price: "12.99 - 14.99",
    description:
      "Chicken or Shrimp, Coconut Milk, Lemon Leaf, Tomato, Mushroom, Thai Pepper, Thai seasoning",
    image: "./images/tomKha.webp",
    alt: "Tom Kha image",
    category: "soups",
  },
  {
    title: "Tom Yum (Tofu Optional)",
    price: "12.99 - 14.99",
    description:
      "Chicken or Shrimp, Coconut Milk, Tomato, Lemon leaf, Thai Pepper, Thai Seasoning",
    image: "./images/tomYum.webp",
    alt: "Tom Yum image",
    category: "soups",
  },
  {
    title: "Egg Rolls",
    price: "3 for 6.00",
    description:
      "Pork, Carrots, cabbage, vermicelli noodles, Thai seasonings. ***Comes with In-house made dipping sauce",
    image: "./images/eggRolls.webp",
    alt: "Egg Rolls image",
    category: "sides",
  },
  {
    title: "Spring Rolls",
    price: "3 for 6.00",
    description:
      "Pork, Carrots, cabbage, vermicelli noodles, Thai seasonings. ***Comes with In-house made dipping sauce",
    image: "./images/springRolls.webp",
    alt: "Spring Rolls image",
    category: "sides",
  },
  {
    title: "Chicken Fried Rice",
    price: "11.99",
    description:
      "Chicken, Rice, Egg, Carrots, Green Onions, Thai Seasoning, Thai Sauces",
    image: "./images/ChickenFriedRice.webp",
    alt: "Chicken Fried Rice image",
    category: "sides",
  },
  {
    title: "Shrimp Fried Rice",
    price: "13.99",
    description:
      "Shrimp, Rice, Egg, Carrots, Green Onions, Thai Seasoning, Thai Sauces",
    image: "./images/shrimpFriedRice.webp",
    alt: "Shrimp Fried Rice image",
    category: "sides",
  },
];
